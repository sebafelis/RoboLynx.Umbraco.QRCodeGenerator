...
namespace RoboLynx.Umbraco.QRCodeGenerator.Helpers
{
    ...
    public class TranslationHelper
    {
        ...
        private static string AnalyticsLangPath = $"~/App_Plugins/{Constants.PluginName}/Config/Lang/";

        public IEnumerable<FileInfo> GetUmbracoLanguageFiles()
        {
        ...
        public IEnumerable<FileInfo> GetAnalyticsLanguageFiles()
        {
        ...
        public IEnumerable<FileInfo> GetUmbracoLanguageFilesToInsertLocalizationData()
        {
        ...
        public void AddTranslations()
        {
            ...
            LogHelper.Info<TranslationHelper>(string.Format("{0} {1} Plugin language files to be merged into Umbraco language files", analyticsFiles.Count(), Constants.PluginName));
            ...
            LogHelper.Info<TranslationHelper>(string.Format("{0} Umbraco language files that match up with our Reports {1} language files", existingLangs.Count(), Constants.PluginName));
            ...
            foreach (var lang in existingLangs)
            {
                ...
                {
                    LogHelper.Error<TranslationHelper>($"Failed during add {Constants.PluginName} Plugin localization values to language file", ex);
                }
        ...
        {
            var analyticsFiles = GetAnalyticsLanguageFiles();
            LogHelper.Info<TranslationHelper>(string.Format("{0} {1} Plugin language files to be removed into Umbraco language files", analyticsFiles.Count(), Constants.PluginName));
            ...
            //Check which language filenames that we have match up
            var existingLangs = GetUmbracoLanguageFilesToInsertLocalizationData();
            LogHelper.Info<TranslationHelper>(string.Format("{0} Umbraco language files that match up with our {1} language files", existingLangs.Count(), Constants.PluginName));
