...
using static RoboLynx.Umbraco.QRCodeGenerator.ImageSharp.ImageSharpSvgQRCode;
using SixLabors.ImageSharp;
using SixLabors.ImageSharp.PixelFormats;

namespace RoboLynx.Umbraco.QRCodeGenerator.ImageSharp
{
    public class ImageSharpSvgQRCode : AbstractQRCode, IDisposable
    {
        /// <summary>
        /// Constructor without params to be used in COM Objects connections
        /// </summary>
        public ImageSharpSvgQRCode() { }
        public ImageSharpSvgQRCode(QRCodeData data) : base(data) { }
        ...
        {
            return this.GetGraphic(viewBox, ColorToHtml(darkColor), ColorToHtml(lightColor), drawQuietZones, sizingMode, logo);
        }
        ...
        public string GetGraphic(Size viewBox, string darkColorHex, string lightColorHex, bool drawQuietZones = true, SizingMode sizingMode = SizingMode.WidthHeightAttribute, SvgLogo? logo = null)
        {
            ...
            StringBuilder svgFile = new($@"<svg version=""1.1"" baseProfile=""full"" shape-rendering=""crispEdges"" {svgSizeAttributes} xmlns=""http://www.w3.org/2000/svg"" xmlns:xlink=""http://www.w3.org/1999/xlink"">");
            ...
            //Render logo, if set
            if (logo != null && logoAttr.HasValue)
            {
                if (!logo.IsEmbedded())
                {
...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
                        var orginalWidth = svg.Root.Attribute("width")?.Value;
#pragma warning restore CS8602 // Dereference of a possibly null reference.
                        var orginalHeight = svg.Root.Attribute("height")?.Value;
                        ...
                        if (string.IsNullOrEmpty(svg.Root.Attribute("viewBox")?.Value) && !string.IsNullOrEmpty(orginalWidth) && !string.IsNullOrEmpty(orginalHeight))
                        {
                            svg.Root.SetAttributeValue("viewBox", $"0 0 {orginalWidth} {orginalHeight}");
                        }
        ...
        private static string ColorToHtml(Color color)
        {
            ...
            return $"rgba({pixel.R},{pixel.G},{pixel.B},{((float)pixel.A / 255f):0.##})";
        }
    ...
    public static class ImageSharpSvgQRCodeHelper
    {
        public static string GetQRCode(string plainText, int pixelsPerModule, string darkColorHex, string lightColorHex, QRCoder.QRCodeGenerator.ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1, bool drawQuietZones = true, SizingMode sizingMode = SizingMode.WidthHeightAttribute, SvgLogo? logo = null)
        {
            ...
            using var qrCode = new ImageSharpSvgQRCode(qrCodeData);
